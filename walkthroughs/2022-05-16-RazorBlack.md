...
And unzipping it shows us the files mentioned in chat chat text file:
...
Using **secretsdump** to extract all of the hashes:
...
We didn't try to use **impacket GetUserSPNs** yet, let's do that to see if we can do any **Kerbroasting**. Note: this command requires 2 hashes and you can use the same hash twice separated by a : to replace the redacted value in this command:
...
Our last objective is asking what the top secret is, so I try and search for any files named top and find 3 matching that criteria:
...
Those certainly look promising and is probably what we're after! I download the **c:\program files\top secret\top_secret.png** file to my home system using the evil-winrm download command and then open it up:
