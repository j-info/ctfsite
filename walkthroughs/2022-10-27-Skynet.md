...
And my http server shows the webserver reaching out to my webserver and getting the file:
...
Checking `sudo -l` we can't run it without a password.
...
So this will create a backup of the html directory with the tar command as root. Backup jobs are usually scheduled so I take a look at the crontabs out there and it's listed and running once every minute:
